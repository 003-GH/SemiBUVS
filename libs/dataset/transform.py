...
class Normalize(object):
    ...
    def __init__(self):
        self.mean = np.array([0.3104568867264076, 0.1484092802923775, 0.315876570513083]).reshape([1, 1, 3]).astype(np.float32)
        self.std = np.array([0.14789415473804174, 0.31522794431189466, 0.14809885107286802]).reshape([1, 1, 3]).astype(np.float32)
