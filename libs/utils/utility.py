...
def save_checkpoint(state, epoch, checkpoint='checkpoint', filename='recurrent'):
...
def write_mask(mask, info, opt, directory='results', model_name=''):
    ...
    for t in range(mask.shape[0]):
        ...
        m = m.transpose((1, 2, 0))
        rescale_mask = cv2.resize(m, (w, h), interpolation=cv2.INTER_CUBIC)
        ...
        if opt.save_indexed_format == 'index':
            binarizer = Binarizer(threshold=0.5).fit(rescale_mask)
            rescale_mask = np.rint(binarizer.transform(rescale_mask))

            rescale_mask = rescale_mask.astype(np.uint8)
