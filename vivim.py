...
class MLP(nn.Module):
    def __init__(self, input_channels=768, num_class=128):
        super().__init__()
...
class Vivim(nn.Module):
    def __init__(
        ...
        if pretrained_segformer is None:
            raise ValueError('pretrained_segformer must be provided')
        backbone = SegformerForSemanticSegmentation.from_pretrained(pretrained_segformer)
        ...
        self.out1 = nn.Conv2d(768, out_chans, kernel_size=1)
    ...
    def decode(self, encoder_hidden_states, bz, nf):
        ...
        projector_result = self.head(hidden_states)

        projector_result1 = self.out1(hidden_states)

        return logits, projector_result, projector_result1

    def forward(self, x_in):
        bz, nf, nc, h, w = x_in.shape
        outs = self.encoder(x_in)
        logits, projector_result, projector_result2 = self.decode(outs,bz,nf)

        upsampled_logits = nn.functional.interpolate(
                logits, size=(h,w), mode="bicubic", align_corners=False
            )

        if self.with_edge:
            edge = self.edgeocr_cls_head(outs[0])
            edge = nn.functional.interpolate(
                    edge, size=(h,w), mode="bicubic", align_corners=False
                )
            return upsampled_logits, edge
        else:

            return upsampled_logits, projector_result, projector_result2
